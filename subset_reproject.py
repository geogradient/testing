...
destDir = '/media/jobel/work/tests/'
srcDir  = '/media/jobel/SeagateDrive/eodata2014/level2/MEGS/' # gpt_configuration.srcDirs['CCL1P_out']
...
for merisFile in srcList:
    ...
    if (merisYear in ["2008","2010","2011"]):
        newList.append(merisFile)
        print("Keep in this one")
    filterList = newList[10:] # Just momentary because it stoped
 
for merisFile in filterList:
    
    merisFileName = os.path.basename(merisFile) # MERIS filename extraction
    inputFileName = merisFile # Keep the full path and current MERIS filename
    outputFile = destDir +'UTM33_WGS84_MEGS_' +merisFileName # New filename
